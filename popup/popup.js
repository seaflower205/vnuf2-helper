...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function renderAccounts() {
    ...
    accounts.forEach(acc => {
      ...
      btnEye.setAttribute('aria-label', `Hiện hoặc ẩn mật khẩu của tài khoản ${acc.username}`);
      btnEye.textContent = '👁';

      const btnStar = document.createElement('button');
      btnStar.className = 'btn-star';
      btnStar.title = 'Đặt mặc định';
      btnStar.setAttribute('aria-label', `Đặt tài khoản ${acc.username} làm mặc định`);
      btnStar.textContent = acc.isDefault ? '★' : '☆';

      const btnLogin = document.createElement('button');
      btnLogin.className = 'btn-login';
      btnLogin.title = 'Đăng nhập';
      btnLogin.setAttribute('aria-label', `Đăng nhập bằng tài khoản ${acc.username}`);
